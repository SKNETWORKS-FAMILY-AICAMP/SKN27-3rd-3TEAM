...
| LLM | Hugging Face Router 기반 Qwen 계열 모델로 자연어 해설 생성 |
...
flowchart LR
    ...
    subgraph EXT["External Model"]
        HF["Hugging Face Router<br/>Qwen 계열 모델"]
    end
...
## 6. Graph-guided Hybrid RAG 내부 구성도

> 기준 흐름은 `backend/team_build_rag/workflow.py`의 실제 LangGraph 실행 순서입니다.  
> 단, 문서에서는 Graph DB 계산 근거와 Vector DB 검색 근거가 어떻게 합쳐지는지 더 잘 보이도록 좌우 영역으로 분리해 표현합니다.
...
|---|---:|---:|---|
| 팀 분석 | 70% | 30% | 타입 상성 계산이 핵심이며, 문서 근거는 설명 보강 역할 |
| 포켓몬 추천 | 80% | 20% | 추천 순위는 약점 보완, 타입 저항, 종족값, 기술 커버리지 계산이 가장 중요 |
| AI 해설 생성 | 60% | 40% | 계산 근거를 우선 신뢰하되, 문서 근거로 사용자가 납득할 수 있는 설명을 보강 |
...
백엔드는 PostgreSQL과 Neo4j에 각각 접근하며, AI 해설 생성 시 외부 Hugging Face Router API를 호출한다.
...
| RAG → Hugging Face | 토큰 누락, 추론 크레딧 부족, 모델 권한/호출 오류 | Hugging Face 토큰, Billing, 모델 설정 확인 |
