...
- 설명 비중은 Graph DB 계산 근거 60%, Vector DB 검색 근거 40%로 둔다.
...
```mermaid
flowchart TD
    START(["LangGraph 상태 수신"])
    ANSWER["answer_generator<br/>프롬프트 생성 시작"]
    TYPE{"request_type 확인"}
    ANALYSIS["_build_analysis_prompt<br/>덱 분석 프롬프트 선택"]
    RECOMMEND["_build_recommendation_prompt<br/>추천 프롬프트 선택"]
    TEMPLATE["프롬프트 템플릿 결정<br/>분석용 또는 추천용"]

    subgraph GRAPH_CONTEXT["계산 결과 입력"]
        GRAPH_RESULT["graph_result 또는 reranked_result<br/>약점, 저항, 점수, 추천 순위"]
        COMPACT["_safe_json<br/>LLM 입력용 JSON 정리"]
    end

    subgraph VECTOR_CONTEXT["검색 근거 입력"]
        VECTOR_DOCS["vector_documents<br/>설명, 기술, 전략 근거"]
        EVIDENCE["_build_evidence_context<br/>근거 문서 최대 8개 정리"]
    end

    RULES["답변 규칙 포함 템플릿 준비<br/>결론 우선, 한국어, 근거 기반"]
    PROMPT["최종 프롬프트 조립<br/>역할 + 계산 결과 + 검색 근거 + 규칙"]
    ...
    GRAPH_RESULT --> COMPACT
    VECTOR_DOCS --> EVIDENCE
    COMPACT --> PROMPT
    EVIDENCE --> PROMPT
...
| 상황 | 처리 원칙 |
|---|---|
| Hugging Face 토큰 없음 | 명확한 오류 메시지로 HF 토큰 설정 필요성을 알린다. |
