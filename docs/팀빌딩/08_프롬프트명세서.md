...
```mermaid
flowchart TD
    A["팀 분석&추천 요청"] --> B["LangGraph answer_generator 진입"]
    B --> C{"request_type 확인"}
    C -- "analysis" --> D["_build_analysis_prompt"]
    C -- "recommendation" --> E["_build_recommendation_prompt"]
    D --> F["Graph DB 계산 결과 삽입"]
    E --> F
    F --> G["Vector DB 검색 근거 삽입"]
    G --> H["답변 규칙 및 금지사항 삽입"]
    H --> I["Hugging Face Router로 LLM 호출"]
    I --> J["final_answer 반환"]
```
