...
**피피고(Pipigo)**는 Chrome 브라우저에 설치하는 확장 프로그램입니다.  
선택한 포켓몬 캐릭터가 모든 웹 페이지 위에서 살아 움직이며, 클릭 한 번으로 말풍선을 열어 텍스트를 입력하면 Groq AI(LLaMA 3.1)가 한국어↔영어 번역 또는 질문 답변을 즉시 돌려줍니다.
...
| 기술 | 파일 | 역할 |
|---|---|---|
| HTML5 | `popup.html` | 확장 아이콘 클릭 시 나타나는 제어 팝업 UI |
| CSS3 | `content.css` | 포켓몬 컨테이너·말풍선·로딩 애니메이션 스타일 |
| Vanilla JavaScript | `content.js` | 웹 페이지에 주입되는 포켓몬 UI + 물리 엔진 |
| Vanilla JavaScript | `popup.js` | 팝업 UI 이벤트 처리 및 설정 저장 |
| Vanilla JavaScript | `background.js` | 서비스 워커 — Groq API 호출 담당 |

### 외부 API / 플랫폼

| 기술 | 용도 |
|---|---|
| **Groq AI API** | LLaMA 3.1 8B Instant 모델을 통한 번역 및 질의응답 |
| **Chrome Extension API** | `chrome.storage.local`, `chrome.runtime`, `chrome.tabs` |
...
포켓몬을 클릭하면 말풍선이 열립니다. 텍스트를 입력하고 `Enter`를 누르면 `background.js`가 Groq API를 호출합니다.

**시스템 프롬프트 (background.js:44)**
...
- 언어 자동 감지: 한국어 입력 시 영어로, 영어 입력 시 한국어로 자동 번역
...
목표 좌표 = 현재 위치 ± (랜덤 600px, 500px) 범위 내 무작위 좌표
화면 경계 = max(50, min(innerWidth - 150, targetX))
```
...
3. 번역하거나 질문할 텍스트를 입력하고 `Enter`를 누릅니다
...
새 기능을 추가하거나 버그를 수정할 때 아래 가이드를 따릅니다.

### 새 포켓몬 파트너 추가

1. GIF 파일을 `img/` 폴더에 추가 (`5.gif` 등)
2. `content.js`의 `PK_IMG_MAP` 객체에 항목 추가
3. `popup.html`의 `.pk-list`에 `<img>` 태그 추가
4. `manifest.json`의 `web_accessible_resources`에 파일명 패턴 확인 (현재 `img/*.gif`로 전체 포함)

### 말투 필터 추가

`background.js`의 `TONE_FILTERS` 객체에 파트너별 변환 함수를 등록합니다.

```javascript
const TONE_FILTERS = {
    "야돈(콧물)": (text) => text + "...천천히...",
    "리자몽(비행)": (text) => `불꽃처럼! ${text}!`
};
```

### 메시지 타입 추가

새로운 팝업↔콘텐츠 스크립트 간 기능을 추가할 때:

1. `popup.js`에서 `sendMessageToContent({ type: "NEW_TYPE", ... })` 호출
2. `content.js`의 `chrome.runtime.onMessage.addListener` 내에 해당 타입 처리 추가

---

## 9. 라이선스 (License)

본 프로젝트는 SKN27-3rd-3TEAM 내부 프로젝트용으로 제작되었습니다.  
포켓몬 캐릭터 및 관련 저작물의 권리는 Nintendo / Creatures Inc. / GAME FREAK inc. 에 있습니다.
