...
| 5 | Node 설계 | `Pokemon`, `Type`, `Move`, `Ability`, `Species`, `Item`, `Nature`, `Generation` 노드를 정의합니다. |
...
배틀 기능은 이후 `Ability`, `Item`, `Nature`, `Team`, `TeamMember`, 메가진화 관계까지 확장합니다.
...
| `natures.json` | 사용 | Nature 노드 생성, 배틀 개체 성격 보정에 사용 |
...
이 프로젝트에서는 `Pokemon`, `Type`, `Move`, `Ability`, `Species`, `Item`, `Nature`, `Generation`을 기본 노드로 봅니다.
...
성격은 배틀에서 능력치 보정에 사용됩니다. 특정 성격은 한 능력치를 1.1배 올리고, 다른 능력치를 0.9배 낮춥니다.

중요한 점은 성격이 `Pokemon` 자체의 정보가 아니라 실제 팀에 들어간 개체의 세팅이라는 점입니다.

따라서 아래 관계는 권장하지 않습니다.

```text
(Pokemon)-[:HAS_NATURE]->(Nature)
```

대신 실제 배틀 세팅에서는 아래 관계를 사용합니다.

```text
(TeamMember)-[:HAS_NATURE]->(Nature)
```
...
| 파일 | 사용 필드 |
| --- | --- |
| `natures.json` | `id`, `name`, `increased_stat`, `decreased_stat` |

### Properties

| property | 설명 | 예시 |
| --- | --- | --- |
| `nature_id` | PokeAPI 기준 성격 ID | `1` |
| `name` | 성격 이름 | `"노력"` |
| `increased_stat` | 상승하는 능력치 | `"attack"` |
| `decreased_stat` | 하락하는 능력치 | `"sp_attack"` |

### 팀 추천에서 사용

초기 팀 추천에서는 직접 사용하지 않습니다.

추천은 포켓몬 종/폼 기준으로 이루어지고, 성격은 실제 개체 세팅에 가까운 정보이기 때문입니다.

### 배틀에서 사용

배틀에서는 매우 중요합니다.

예를 들어 실제 팀 멤버가 공격 상승, 특수공격 하락 성격을 가진다면 물리 기술 데미지는 유리하고 특수 기술 데미지는 불리해집니다.

배틀 계산에서는 아래 흐름으로 사용합니다.

```text
TeamMember
-> HAS_NATURE
-> Nature
-> increased_stat / decreased_stat 확인
-> 실제 능력치 계산에 1.1 / 0.9 보정 적용
```
...
## 6.4.8 TeamMember - HAS_NATURE -> Nature

### 목적

실제 팀 멤버가 선택한 성격을 나타냅니다.

성격은 포켓몬 도감 데이터가 아니라 실제 배틀 개체의 세팅입니다. 그래서 `Pokemon`에 직접 연결하지 않고 `TeamMember`에 연결합니다.
...
능력치 계산 시 상승 능력치에는 1.1배, 하락 능력치에는 0.9배 보정을 적용합니다.

예를 들어 `Nature.increased_stat = "attack"`이고 `Nature.decreased_stat = "sp_attack"`이면 물리 공격은 유리하고 특수 공격은 불리해집니다.
...
배틀은 도감 데이터만으로는 완성되지 않습니다.

아래 표는 배틀 기능에서 필요한 데이터가 현재 그래프에 반영되어 있는지 확인하기 위한 체크리스트입니다.

| 항목 | 현재 반영 상태 | 설명 |
| --- | --- | --- |
| 포켓몬 기본 능력치 | 반영 | `Pokemon` 노드의 `hp`, `attack`, `defense`, `sp_attack`, `sp_defense`, `speed` |
| 포켓몬 타입 | 반영 | `Pokemon - HAS_TYPE -> Type` |
| 기술 타입/위력/명중률 | 일부 반영 | `Move` 노드와 `Move - HAS_TYPE -> Type` |
| 기술 PP/우선도/상태 효과 | 미반영 | 현재 `moves.json`에 없음. 필요하면 processor에서 추가 추출 필요 |
| 특성 | 구조 반영 | `Ability` 노드와 `Pokemon - CAN_HAVE -> Ability` |
| 실제 선택 특성 | 추후 반영 | `TeamMember - HAS -> Ability` |
| 아이템 | 구조 반영 | `Item` 노드 |
| 실제 장착 아이템 | 추후 반영 | `TeamMember - HOLDS -> Item` |
| 성격 | 반영 | `Nature` 노드 |
| 실제 선택 성격 | 추후 반영 | `TeamMember - HAS_NATURE -> Nature` |
| 레벨 | 추후 반영 | `TeamMember.level` 속성 |
| 개체값 IV | 미반영 | 실전 배틀 계산 시 `TeamMember` 속성으로 추가 가능 |
| 노력치 EV | 미반영 | 실전 배틀 계산 시 `TeamMember` 속성으로 추가 가능 |
| 현재 HP | 추후 반영 | 배틀 상태에서 `TeamMember.current_hp` 또는 BattleMember 노드로 관리 |
| 상태 이상 | 미반영 | `StatusCondition` 노드 또는 `TeamMember.status` 속성 후보 |
| 랭크 변화 | 미반영 | 공격/방어/스피드 랭크 등 배틀 중 상태 |
| 날씨 | 미반영 | `Weather` 노드 또는 `BattleState.weather` 속성 후보 |
| 필드 | 미반영 | `FieldEffect` 노드 또는 `BattleState.field` 속성 후보 |
| 메가진화/폼 변화 | 설계만 반영 | `CAN_MEGA_EVOLVE_TO`, `MEGA_REQUIRES`는 추가 데이터 필요 |

정리하면 현재 1차 Graph DB는 정적 포켓몬 지식 그래프입니다.

실제 배틀 계산에 필요한 개체 세팅과 턴 상태는 `TeamMember`, `Battle`, `BattleState` 계층에서 추가해야 합니다.
...
| `TeamMember - HAS_NATURE -> Nature` | 실제 선택 성격 |
...
| `TeamMember - HAS_NATURE -> Nature` | 높음 | 실제 성격 보정 |
