...
from mypage.api import (
    get_pokemon_names, _b64_img, fetch_user_stats, fetch_user_logs,
    fetch_team_history, fetch_chatbot_sessions, fetch_github_details,
)
from mypage.constants import TIER_POKEMON, TIER_FRIENDS, GYM_LEADERS
...
def show():
    ...
    if gh:
        repos, commits, stars, followers = gh["repos"], gh["commits"], gh["stars"], gh["followers"]
    ...
    <div class="mp-card card-profile mp-hero">
        ...
        <div class="mp-hero-creature-wrap">
            <div class="mp-hero-friends"><img src="{hero_friends}" alt="friend-pokemon"></div>
            <div class="mp-hero-creature"><img src="{hero_pkmn}" alt="tier-pokemon"><div class="mp-creature-glow"></div></div>
        </div>
    </div>
    ...
    <div class="mp-card card-dev">
        <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px;">
            <div><div class="mp-stat-label">Commits</div><div class="mp-stat-value" style="color:#6c5ce7; font-size: 2.2rem;">{commits:,}</div></div>
            <div><div class="mp-stat-label">Repos</div><div class="mp-stat-value" style="color:#2ecc71; font-size: 2.2rem;">{repos}</div></div>
            <div><div class="mp-stat-label">Stars</div><div class="mp-stat-value" style="color:#f1c40f; font-size: 2.2rem;">{stars}</div></div>
            <div><div class="mp-stat-label">Followers</div><div class="mp-stat-value" style="color:#0984e3; font-size: 2.2rem;">{followers}</div></div>
        </div>
    ...
    KANTO_BADGES = [
        {"file": "회색배지.png",   "name": "볼더 배지",      "gym": "회색시티 · 브록",    "mission": "트레이너 등록",   "desc": "마이페이지 방문",                      "glow": "#a0aec0", "unlocked": True},
        {"file": "블루배지.png",   "name": "캐스케이드 배지", "gym": "하늘색시티 · 미스티", "mission": "퀴즈 첫 도전",    "desc": f"실루엣 퀴즈 1회+  ({s_total}회)",      "glow": "#63b3ed", "unlocked": s_total >= 1},
        {"file": "골드배지.png",   "name": "썬더 배지",       "gym": "갈색시티 · 마티스",   "mission": "베테랑 퀴즈러",   "desc": f"퀴즈 정답 5회+  ({s_correct}회)",      "glow": "#f6e05e", "unlocked": s_correct >= 5},
        {"file": "무지개배지.png", "name": "레인보우 배지",   "gym": "무지개시티 · 민화",   "mission": "도감 컬렉터",     "desc": f"포켓몬 10마리 수집  ({len(_collected_ids)}마리)", "glow": "#f687b3", "unlocked": len(_collected_ids) >= 10},
        {"file": "핑크배지.png",   "name": "소울 배지",       "gym": "연분홍시티 · 독수",   "mission": "팀 분석가",       "desc": f"팀 분석 1회 진행  ({len(team_history)}회)",      "glow": "#ed64a6", "unlocked": len(team_history) >= 1},
        {"file": "진홍색배지.png", "name": "볼케이노 배지",   "gym": "홍련섬 · 강연",       "mission": "오박사와 대화",   "desc": f"오박사님과 대화 1회+  ({len(chat_sessions)}회)",  "glow": "#fc8181", "unlocked": len(chat_sessions) >= 1},
        {"file": "오렌지배지.png", "name": "마쉬 배지",       "gym": "노랑시티 · 초련",     "mission": "배틀 도전자",     "desc": f"배틀 참여 1회+  ({b_total}회)",         "glow": "#ed8936", "unlocked": b_total >= 1},
        {"file": "그린배지.png",   "name": "어스 배지",       "gym": "크리스탈 · 라이벌",   "mission": "챔피언 도전자",   "desc": f"트레이너 Lv.10+  (Lv.{level})",        "glow": "#68d391", "unlocked": level >= 10},
    ]
    ...
    st.markdown(f"""<div class="mp-card card-badge" style="padding:0 !important; overflow:hidden; border-radius: 28px; border:none;"><div class="badge-case" style="border:none; border-radius:0; padding: 40px;"><div class="badge-case-title">KANTO REGION · BADGE CASE</div><div class="badge-case-grid">{slots_html}</div><div class="badge-case-footer" style="margin-top:30px;">{footer_msg}</div></div></div>""", unsafe_allow_html=True)

    gym_badges_owned = stats.get("gym_badges", []) if stats else []
    gym_slot_parts = []
    for g in GYM_LEADERS:
        b64     = _b64_img(f"img/badge/battle/{g['id']}_뱃지.png")
        glow    = g["glow"]
        label   = g["label"]
    ...
    <div class="mp-card card-game">
        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 20px;">
            <div><div class="mp-stat-label">Quiz Accuracy</div><div class="mp-stat-value" style="color:#ff4757;">{s_rate}%</div></div>
            <div><div class="mp-stat-label">Memory Record</div><div class="mp-stat-value" style="color:#e67e22;">{m_rate}%</div></div>
        </div>
    ...
    if not collected_ids:
        ...
        st.markdown(f"""<div class="mp-card card-dex"><div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(100px, 1fr)); gap: 15px;">{dex_grid}</div><div style="margin-top: 35px; padding-top: 25px; border-top: 1px solid rgba(0,0,0,0.05);"><div style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 12px;"><div><div style="font-size: 0.8rem; font-weight: 800; color: #2d3436; opacity: 0.7; margin-bottom: 4px;">COLLECTION RATE</div><div style="font-size: 1.8rem; font-weight: 900; color: #27ae60;">{collected_count} <span style="font-size: 1rem; color: #636e72;">/ {total_pkmn_count}</span></div></div><div style="text-align: right;"><div style="font-size: 1.8rem; font-weight: 900; color: #27ae60;">{capture_rate:.1f}%</div><div style="font-size: 0.75rem; font-weight: 700; color: #636e72;">{total_pkmn_count - collected_count} Left to Complete</div></div></div><div class="mp-xp-track" style="height: 12px; background: rgba(0,0,0,0.05);"><div class="mp-xp-fill" style="width: {capture_rate}%; background: linear-gradient(90deg, #2ecc71, #27ae60); box-shadow: 0 0 15px rgba(39, 174, 96, 0.4);"></div></div></div></div>""", unsafe_allow_html=True)
    ...
    if not logs:
        ...
        for log in logs:
            ...
            activity_items.append(
                f'<div class="mp-log-item"><div class="mp-log-icon">{g_icon}</div>'
                f'<div style="flex: 1;"><div class="mp-log-main">{g_name} <span class="mp-tag {tag_cls}">{tag_txt}</span></div>'
                f'<div class="mp-log-sub">대상 포켓몬: {p_name}</div></div>'
                f'<div style="font-size: 0.8rem; color: #636e72; font-weight:600;">{time_str}</div></div>'
            )
    st.markdown(f'<div class="mp-card card-activity">{"".join(activity_items)}</div>', unsafe_allow_html=True)

    st.markdown('<div class="mp-section-title">Team Builder History</div>', unsafe_allow_html=True)
    if not team_history:
        st.markdown("""<div class="mp-card card-history" style="text-align:center; padding: 60px;"><div style="font-size: 3rem; margin-bottom: 20px;">📜</div><div style="font-weight: 800; color: #2d3436; font-size: 1.2rem;">팀 빌더 분석 기록이 없습니다.</div><div style="color: #636e72; margin-top: 10px;">팀을 구성하고 분석해보세요!</div></div>""", unsafe_allow_html=True)
    else:
        with st.container():
            ...
            for i, log in enumerate(team_history):
                ...
                with row_col:
                    ...
                    sel_imgs_html = "".join([f'<img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/{pid}.png" class="mp-hist-pkmn-img" alt="{pid}">' for pid in sel_ids[:5]])
                    rec_imgs_html = "".join([f'<img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/{pid}.png" class="mp-hist-pkmn-img mp-hist-pkmn-rec" alt="{pid}">' for pid in rec_ids[:3]]) if rec_ids else '<span style="color:#a0aec0;font-size:0.75rem;">없음</span>'
                    ...
                    conclusion_html = (f'<div class="mp-hist-conclusion-h">{conclusion}</div>' if conclusion else '<div class="mp-hist-conclusion-h" style="color:#4a5568;font-style:italic;">분석 결과 텍스트 없음</div>')
                    st.markdown(
                        '<div class="mp-hist-card-h"><div class="mp-hist-team-block">'
                        f'<div class="mp-hist-date">{date_str}</div>'
                        '<div class="mp-hist-label">나의 팀</div>'
                        f'<div class="mp-hist-row">{sel_imgs_html}</div></div>'
                        f'<div class="mp-hist-sep"></div><div class="mp-hist-text-block">{conclusion_html}</div>'
                        f'<div class="mp-hist-sep"></div><div class="mp-hist-rec-block">'
                        '<div class="mp-hist-label">추천 포켓몬</div>'
                        f'<div class="mp-hist-row">{rec_imgs_html}</div></div></div>',
                        unsafe_allow_html=True,
                    )
                with btn_col:
                    ...
                    if st.button("결과 보기", key=f"hist_{log['id']}", disabled=not has_result, use_container_width=True):
